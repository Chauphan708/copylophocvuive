...
const AddTeamModal: React.FC<AddTeamModalProps> = ({ isOpen, onClose, onAddTeam }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (name.trim()) {
      onAddTeam(name.trim(), selectedColor);
      setName('');
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <h2 className="text-2xl font-bold text-slate-800 text-center mb-6">Thêm Tổ Mới</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ví dụ: Tổ 1 - Đoàn kết"
                  required
                />
              </div>
              <div>
